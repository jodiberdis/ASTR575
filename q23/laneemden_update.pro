...
pro laneemden
  ...
  h = 0.00001                   ; step size for Runge-Kutta
  xsi = 0.00001                 ; initial value of xsi (can't be exactly 0)
  
; Comment: Several step-size values were tested. 0.001 ran immediately, 0.0001 took approximately 10 seconds,
;   and 0.00001 took over 10 minutes. I am slightly hesitant about trying 0.000001, as I fear it may take up too much time,
;   so I've decided to stop at 0.00001.

  ; Determine initial values of y and z (can't be exactly 0) -- comes from expansion of Lane-Emden eqn:
  y = 1. - (1./6.)*(xsi^2.) + (n/120.)*(xsi^4.) - (n*(8.*n - 5.)/15120.)*(xsi^6.)
  z = (-(1./3.)*xsi) + (n/30.)*(xsi^3.) - (n*(8.*n - 5.)/2520.)*(xsi^5.)

  xsiarr = [xsi]               ; set up blank array for xsi values
  yarr = [y]                   ; set up blank array for y values
  zarr = [z]                   ; set up blank array for z values
  ...
  WHILE y GE 0 do begin
    ...
    y = y + (1./6.)*(k1 + (2.*k2) + (2.*k3) + k4)    ; increment y and z by a tiny amount from 
    z = z + (1./6.)*(l1 + (2.*l2) + (2.*l3) + l4)    ;    k1,k2,k3,k4, l1,l2,l3,l4 before returing to top
  ...
  xsi_1 = xsiarr(-1)         ; xsi at the surface (y=0)
  z_1 = zarr(-1)             ; z at the surface (y=0)
  ...
  Nn = (((4.*pi)^(1./n))/(n+1.))*(-(xsi_1^((n+1)/(n-1)))*z_1)^((1.-n)/n)   ; N coefficient
